...
describe("middleware", () => {
  it("re-exports NextAuth's `auth` as the middleware handler", () => {
  ...
  it("runs the middleware on application routes", () => {
    ...
    // Non-auth API routes should still hit middleware.
    expect(matchesMiddlewarePath("/api/health")).toBe(true)
